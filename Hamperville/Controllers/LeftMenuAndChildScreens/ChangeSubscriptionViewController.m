...
- (void)initialSetup {
    ...
    [self.statusSwitch setOn:self.status animated:NO];
    ...
    for (NSDictionary *subscription in self.allSubscriptions) {
        if ([[[subscription valueForKey:@"subscription_plan"] valueForKey:@"id"] integerValue] == self.currentSubscriptionPlanID) {
            ...
            self.selectLabel.text = [[subscription valueForKey:@"subscription_plan"] valueForKey:@"amount"];
            break;
        }
        count++;
    }
...
- (void)backButtonTapped {
    if (self.status) {
        [self.updateDelegate updateViewsWithStatus:self.status subscriptionID:self.currentSubscriptionPlanID andNextRenewalDate:self.nextRenewalDate];
    } else {
        [self.updateDelegate updateViewsWithStatus:self.status];
    }
...
- (IBAction)saveButtonTapped:(id)sender {
    if (self.status == true && self.currentSubscriptionPlanID == 0) {
        [self showToastWithText:@"Select plan" on:Top];
        return;
    }
    [self.activityIndicator startAnimating];
    [[RequestManager alloc]postSubscriptionWithStatus:self.status andSubscriptionID:[NSString stringWithFormat:@"%ld",self.currentSubscriptionPlanID] withCompletionBlock:^(BOOL success, id response) {
        [self.activityIndicator stopAnimating];
        if (success) {
            if ([response hasValueForKey:@"subscription_status"]) {
                self.status = [[response valueForKey:@"subscription_status"] boolValue];
                if (self.status) {
...
- (IBAction)doneButtonTapped:(id)sender {
    ...
    self.selectLabel.text = [[self.allSubscriptions[[self.pickerView selectedRowInComponent:0]] valueForKey:@"subscription_plan"] valueForKey:@"amount"];
